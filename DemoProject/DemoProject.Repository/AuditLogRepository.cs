...
namespace DemoProject.Repository
{
    public class AuditLogRepository : BaseRepository, IAuditLogRepository
    {
        ...
        public AuditLogRepository(IUnitOfWork unitOfWork, IServiceScopeFactory serviceScopeFactory, IHttpContextAccessor httpContext)
        {
        ...
        public async Task SaveAuditLogs(int moduleId, string FormName, string CalledFunction, int actionId, int userRightId, int userTypeId, object logMessage, string logRefMessage, bool IsObject, int logTypeId)
        {
            try
            {
                ...
                _unitOfWork.Repository<AuditLog>().InsertAsync(log);
        ...
        //        {
        //            var db = scope.ServiceProvider.GetService<LogDbContext>();
